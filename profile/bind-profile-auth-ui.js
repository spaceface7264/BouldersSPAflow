// Profile login/edit/logout/forgot-password (uses window.navigateToRoute etc. after init).
// Expects profile context from buildProfileContext plus refreshLoginPageUI & populateProfileEditForm.
export function bindProfileAuthUi(ctx) {
  const {
    DOM,
    state,
    authAPI,
  ...
  if (DOM.loginPageForm) {
    DOM.loginPageForm.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        const isTestMode =
          urlParamsInner.get('test') === 'true' || localStorage.getItem('testMode') === 'true';
        const useMockLogin =
          urlParamsInner.get('mock') === 'true' || localStorage.getItem('mockLogin') === 'true';
        ...
        setTimeout(() => {
          ...
          if (pageContentWrapper) {
            ...
            if (!currentHash || !PAGE_ROUTES.includes(currentHash)) {
        ...
        const isTestMode =
          urlParamsErr.get('test') === 'true' || localStorage.getItem('testMode') === 'true';
  ...
  if (DOM.profileEditForm) {
    DOM.profileEditForm.addEventListener('submit', async (event) => {
      ...
      const submitButtonEdit = DOM.profileEditForm.querySelector('button[type="submit"]');
      const originalTextEdit = submitButtonEdit?.textContent || 'SAVE CHANGES';
      if (submitButtonEdit) {
        submitButtonEdit.disabled = true;
        submitButtonEdit.textContent = 'Saving...';
      }
      ...
      try {
        ...
        if (submitButtonEdit) {
          ...
          submitButtonEdit.textContent = originalTextEdit;
        }
      }
    });
  }
  ...
  if (DOM.forgotPasswordForm) {
    DOM.forgotPasswordForm.addEventListener('submit', async (e) => {
      ...
      const submitButtonFp = DOM.forgotPasswordForm.querySelector('button[type="submit"]');
      const originalTextFp = submitButtonFp?.textContent || 'SEND RESET LINK';
      if (submitButtonFp) {
        submitButtonFp.disabled = true;
        submitButtonFp.textContent = 'Sending...';
      }
      ...
      try {
        ...
        if (submitButtonFp) {
          ...
          submitButtonFp.textContent = originalTextFp;
        }
      }
    });
  }
